...
#     sys.path.append(sys.path[0]+"/tracker")
#     sys.path.append(sys.path[0]+"/tracker/model")
from colmap_tracker import ColmapTrackerSegmentAnything
from colmap_tracker import parse_augment, Image
import requests
import json
import torchvision
import torch 
...
def init_colmap(path):
    tracker.init_colmap(path)
...
def get_frames_from_folder(folder_input, video_state):
    ...
    # assert len(image_paths) == len(tracker.img_ids)
    
    # Check with colmap
    
    for img_info in tracker.imgs_info:
        assert os.path.isfile(os.path.join(folder_input,img_info.name))
    ...
    try:
        ...
        for img_info in tracker.imgs_info:
            image_path = os.path.join(folder_input,img_info.name)
            ...
            assert frame.dtype == frames[0].image.dtype, "Inputs have multiple data types!"
    ...
    image_size = (frames[0].image.shape[0],frames[0].image.shape[1]) 
    ...
    video_state = {
        ...
        "masks": [np.zeros((frames[0].image.shape[0],frames[0].image.shape[1]), np.uint8)]*len(frames),
    ...
    tracker.samcontroler.sam_controler.set_image(video_state["origin_images"][0].image)
    
    # model.samcontroler.sam_controler.reset_image() 
    # model.samcontroler.sam_controler.set_image(video_state["origin_images"][0])
    return video_state, video_info, video_state["origin_images"][0].image, gr.update(visible=True, maximum=len(frames), value=1), gr.update(visible=True, maximum=len(frames), value=len(frames)), \
...
def select_template(image_selection_slider, video_state, interactive_state, mask_dropdown):
    ...
    tracker.samcontroler.sam_controler.reset_image() 
    tracker.samcontroler.sam_controler.set_image(video_state["origin_images"][image_selection_slider].image)
    # model.samcontroler.sam_controler.reset_image()
    ...
    return video_state["painted_images"][image_selection_slider].image, video_state, interactive_state, operation_log
...
def sam_refine(video_state, point_prompt, click_state, interactive_state, evt:gr.SelectData):
    ...
    tracker.samcontroler.sam_controler.reset_image()
    tracker.samcontroler.sam_controler.set_image(video_state["origin_images"][video_state["select_frame_number"]].image)
    # model.samcontroler.sam_controler.reset_image()
    ...
    return painted_image.image, video_state, interactive_state, operation_log
...
def clear_click(video_state, click_state):
    click_state = [[],[]]
    template_frame = video_state["origin_images"][video_state["select_frame_number"]].image
...
def show_mask(video_state, interactive_state, mask_dropdown):
    mask_dropdown.sort()
    select_frame = video_state["origin_images"][video_state["select_frame_number"]].image
...
def vos_tracking_video(video_state, interactive_state, mask_dropdown):
    ...
    masks, logits, painted_images = tracker.generator(images= following_frames)
...
def generate_video_from_frames(frames, output_path, fps=30):
    ...
    # video.release()
    # try:
    images = [np.array(frame.image) for frame in frames]
    frames = torch.from_numpy(np.asarray(images))
    # except:
    #     for frame in frames:
    #         print(frame.dtype)
...
tracker = ColmapTrackerSegmentAnything(SAM_checkpoint, args)
...
with gr.Blocks() as iface:
        ...
        with gr.Column():
            ...
            with gr.Row():
                ...
                with gr.Column():    
                    ...
                    with gr.Column():
                        colmap_folder = gr.Textbox(value = "", label = "Colmap model path")
                        load_colmap_button = gr.Button(value="Load colmap model", interactive=True, variant="primary") 
                        
                        image_folder = gr.Textbox(value = "", label = "Image folder path")
                        load_images_button = gr.Button(value="Load images", interactive=True, variant="primary", visible=False) 
    ...
    # )   
    # get 
    load_colmap_button.click(
        fn = init_colmap,
        inputs=[
            colmap_folder
        ],
        outputs=[load_images_button]
    )
